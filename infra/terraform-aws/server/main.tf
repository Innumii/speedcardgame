...
locals {
  ...
  server_dir   = "${local.project_root}/server"
...
resource "aws_lb_target_group" "server" {
  ...
  health_check {
    path = var.server_healthcheck_path
  }
}
...
resource "null_resource" "build_server_image" {
  ...
  triggers = {
    ...
    repo_url  = aws_ecr_repository.server.repository_url
  }
  ...
  provisioner "local-exec" {
    ...
    command     = "${join("; ", [
      ...
      "docker build -f ${local.server_dir}/dockerfile -t ${aws_ecr_repository.server.repository_url}:${var.image_tag} ${local.server_dir}",
      "docker push ${aws_ecr_repository.server.repository_url}:${var.image_tag}"
    ])}"
  }
}
