...
func ListCards(w http.ResponseWriter, _ *http.Request) {
    ...
    startPage := 0      // Default to first page
    endPage := PageSize // Default to first page size

	// Query for cards ordered by Cid within the specified page range
    if result := config.DB.Order("cid").Offset(startPage).Limit(endPage - startPage).Find(&cards); result.Error != nil {
